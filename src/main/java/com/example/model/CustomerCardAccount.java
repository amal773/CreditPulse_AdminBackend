...
public class CustomerCardAccount {
	...
	public CustomerCardAccount(
			@NotNull(message = "Account number cannot be null") @Min(value = 1, message = "Account number must be positive") Long accountNumber,
			CustomerProfile customerProfile,
			@Size(min = 3, max = 3, message = "Base currency must be a 3-letter currency code") String baseCurrency,
			@PastOrPresent(message = "Opening date must be in the past or present") Date openingDate,
			ActivationStatus activationStatus,
			@Min(value = 1, message = "Card number must be positive") Long cardNumber, ActivationStatus cardStatus,
			PaymentStatus internationalPayment, PaymentStatus onlinePayment,
			@Min(value = 1000, message = "PIN must be a 4-digit number") @Max(value = 9999, message = "PIN must be a 4-digit number") int pin,
			PaymentStatus cardSwipe,
			@DecimalMin(value = "0.0", inclusive = true, message = "Online payment limit must be greater than or equal to 0") BigDecimal onlinePaymentLimit,
			@DecimalMin(value = "0.0", inclusive = true, message = "Card swipe limit must be greater than or equal to 0") BigDecimal cardSwipeLimit,
			@DecimalMin(value = "0.0", inclusive = true, message = "International payment limit must be greater than or equal to 0") BigDecimal internationalPaymentLimit,
			@DecimalMin(value = "0.0", inclusive = true, message = "Card balance must be greater than or equal to 0") BigDecimal cardBalance,
			@DecimalMin(value = "0.0", inclusive = true, message = "Due amount must be greater than or equal to 0") BigDecimal dueAmount,
			Date dueDate, Date expiryDate,
			@Pattern(regexp = "\\d{3}", message = "CVV must be a 3 digit number") String cvv,
			@DecimalMin(value = "0.0", inclusive = true, message = "Credit card limit must be greater than or equal to 0") BigDecimal creditCardLimit,
			String creditCard) {
		super();
		this.accountNumber = accountNumber;
		this.customerProfile = customerProfile;
